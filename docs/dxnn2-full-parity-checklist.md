...
|---|---|---|
| `polis.erl` | `partial` | Core lifecycle + persistence orchestration implemented, including idempotent init/start state, explicit stop lifecycle, scape lookup semantics, configured supporting-module start/stop lifecycle, configured public-scape autostart with tracked scape summary metadata (`type`/`parameters`/`metabolics`/`physics`), explicit `create`/`reset` lifecycle APIs backed by storage reset capability, and CLI exposure via `protogonosctl reset`; OTP process/supervision semantics still simplified. |
