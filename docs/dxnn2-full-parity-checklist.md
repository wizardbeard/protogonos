...
| `plasticity.erl` | `partial` | Runtime plasticity integration + mutation hooks implemented with Hebbian/Oja updates, saturation bounds, reference alias coverage (`hebbian_w`, `ojas`, `ojas_w`, `self_modulationV1..V6`, `neuromodulation`), per-target-neuron rule/rate override precedence, and generalized Hebbian coefficient support (`A/B/C/D`) with neuromodulation dead-zone scaling; `mutate_plasticity_parameters` now mutates self-modulation synapse parameter vectors (`synapse.plasticity_params`, rule-width aware) with coefficient fallback when no incoming synapse candidates exist, and retains rate mutation for non-self-mod rules; default plasticity-rule mutator pools now include self-modulation/neuromodulation names, and runtime self-modulation dynamics now consume both per-synapse vectors (`synapse.plasticity_params`) and per-neuron bias vectors (`neuron.plasticity_bias_params`) to derive dynamic `H`/`A`/`B`/`C`/`D` terms (V1/V4/V6 families); remaining parity gap is fuller reference co-mutation choreography of neural- and weight-parameter vectors. |
