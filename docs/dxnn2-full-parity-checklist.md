...
| `plasticity.erl` | `partial` | Runtime plasticity integration + mutation hooks implemented with Hebbian/Oja updates, saturation bounds, and explicit reference PF coverage (`hebbian`, `hebbian_w`, `oja`/`ojas`, `ojas_w`, `self_modulationV1..V6`, `neuromodulation`); `hebbian_w`/`ojas_w` now use per-synapse learning parameters (`synapse.plasticity_params`) with fallback to rate when absent, while self-modulation rules apply per-target-neuron override precedence plus generalized Hebbian coefficients (`A/B/C/D`) and neuromodulation dead-zone scaling; `mutate_plasticity_parameters` now performs rule-width vector mutation over both synapse vectors (`synapse.plasticity_params`) and bias vectors (`neuron.plasticity_bias_params`) using reference-closer probabilistic perturbation density (`1/sqrt(total_parameters)`), co-mutates neural coefficients for V2/V3/V5 families with rule-specific schedules (`V2: 0.5`, `V3/V5: 1/sqrt(N)`), applies reference-closer neuromodulation parameter-set perturbation scheduling over `[rate,A,B,C,D]` (`1/sqrt(5)`), applies signed single-parameter mutation semantics for `hebbian`/`oja`, and traverses vectors in reference-closer order (incoming synapses first, bias vector last); `mutate_pf` now reseeds/clears rule-specific weight-parameter vectors on PF transitions (reference-style `input_idps` reset analog) and reseeds rule-specific neural-parameter defaults; `none` now correctly cancels `mutate_plasticity_parameters` with `ErrNoMutationChoice` and empty per-neuron rule fields now correctly fall back to genome-level rule defaults; remaining parity gap is narrower around full PF-specific mutation choreography edge-cases. |
