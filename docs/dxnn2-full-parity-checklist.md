...
| `plasticity.erl` | `partial` | Runtime plasticity integration + mutation hooks implemented with Hebbian/Oja updates, saturation bounds, reference alias coverage (`hebbian_w`, `ojas`, `ojas_w`, `self_modulationV1..V6`, `neuromodulation`), per-target-neuron rule/rate override precedence, and generalized Hebbian coefficient support (`A/B/C/D`) with neuromodulation dead-zone scaling; `mutate_plasticity_parameters` now performs rule-width self-modulation vector mutation over both synapse vectors (`synapse.plasticity_params`) and bias vectors (`neuron.plasticity_bias_params`) using reference-closer probabilistic perturbation density (`1/sqrt(total_parameters)`), co-mutates neural coefficients for V2/V3/V5 families with rule-specific probabilistic schedules (`V2: 0.5`, `V3/V5: 1/sqrt(N)`), and retains rate mutation for non-self-mod rules; default plasticity-rule mutator pools include self-modulation/neuromodulation names, and runtime self-modulation dynamics consume both synapse and bias vectors to derive dynamic `H`/`A`/`B`/`C`/`D` terms (V1/V4/V6 families); remaining parity gap is narrower around strict reference parameter-vector ordering and full PF-specific mutation choreography. |
