...
| `exoself.erl` | `partial` | Bounded exoself tuning loop integrated with attempts/policies, minimum-improvement acceptance threshold (`MIN_PIMPROVEMENT`-style gating) now using relative dominance semantics (`candidate > incumbent + incumbent * tune_min_improvement`) with monotonic-improvement guards, and attempt budgeting now follows reference-style consecutive non-improvement counting (counter resets on accepted improvements), plus goal-aware early stop (mapped to `fitness_goal`), perturbation controls (`perturbation_range`, `annealing_factor`), and per-generation tuning telemetry surfaced in diagnostics/artifacts; full orchestration parity still pending. |
