...
| `neuron.erl` | `partial` | Runtime neuron/synapse eval implemented with activation, aggregator modes, and reference-aligned output saturation (`[-1,1]`), with per-target-neuron plasticity rule/rate precedence now supported in runtime weight updates (post-neuron metadata overrides genome defaults) and recurrent synapses now using prior-step output memory when forward state is present (reset on cortex reactivation); explicit runtime backup/restore/perturb protocol support is now exposed via cortex APIs (`BackupWeights`/`RestoreWeights`/`PerturbWeights`), while full OTP inbox/reset message choreography remains pending. |
