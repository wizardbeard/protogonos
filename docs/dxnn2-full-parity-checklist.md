...
|---|---|---|
| `polis.erl` | `partial` | Core lifecycle + persistence orchestration implemented, including idempotent init/start state, explicit stop lifecycle with reason surface (`normal`/`shutdown`) plus tracked last-stop reason, package-level default runtime registry APIs mirroring `whereis(polis)`-style singleton lifecycle (`StartDefault`/`Default`/`StopDefault`), scape lookup semantics by both registered name and configured public scape type (`GetScapeByType`, first-configured type match), configured supporting-module start/stop lifecycle plus online add/remove management (`AddSupportModule`/`RemoveSupportModule`), configured public-scape autostart with tracked scape summary metadata (`type`/`parameters`/`metabolics`/`physics`) plus online add/remove management (`AddPublicScape`/`RemovePublicScape`), explicit `create`/`reset` lifecycle APIs backed by storage reset capability (reset now uses shutdown-style teardown), and CLI exposure via `protogonosctl reset`; OTP process/supervision semantics still simplified. |
