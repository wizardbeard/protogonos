...
| `map2rec.erl` | `partial` | Baseline `constraint`/`pmp`/`sensor`/`actuator` conversion parity implemented, plus `neuron`/`agent` materializers to accelerate `records.hrl` parity; broader record coverage still pending. |
