...
| `tuning_selection.erl` | `partial` | Reference mode-name surface implemented (`dynamic`, `dynamic_random`, `active`, `active_random`, `current`, `current_random`, `all`, `all_random`) plus legacy aliases (`recent`, `recent_random`, `lastgen`, `lastgen_random`); random modes use reference-style probabilistic subset selection (`1/sqrt(N)` with non-empty fallback), age modes use generation-aware filtering inferred from genome IDs, `all`/`all_random` now operate over the full candidate pool (not current-only), and `lastgen*` aliases now resolve to current-generation semantics; mutate-weights selection now applies age-annealed per-neuron spread from tuning mode and neuron generation metadata/ID fallback, while full actuator-inclusive spread behavior remains simplified versus source. |
