...
| `map2rec.erl` | `done` | Reference conversion surface implemented (`constraint`/`pmp`/`sensor`/`actuator`) and extended with broader record-materialization coverage (`experiment`/`neuron`/`agent`/`cortex`/`substrate`/`polis`/`scape`/`sector`/`avatar`/`object`/`circle`/`square`/`line`/`e`/`a`/`specie`/`population`/`trace`/`stat`/`topology_summary`/`signature`/`champion`) with mapping and fallback tests. |
