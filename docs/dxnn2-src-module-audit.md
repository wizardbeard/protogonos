...
| `signal_aggregator.erl` | dot/mult/diff aggregation modes | selectable per-neuron aggregation in forward path with reference-aligned multiplicative `mult_product` behavior; stateful `diff_product` history semantics remain partial | `partial` | `internal/nn/network.go`, `internal/nn/network_test.go`, `.ref/src/signal_aggregator.erl` |
