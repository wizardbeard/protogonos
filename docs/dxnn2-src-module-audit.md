...
| `genome_mutator.erl` | mutation operator suite | `internal/evo/mutations.go` with dedicated bias add/remove, activation/aggregator, and plasticity-rule mutation operators, reference `add_bias`/`remove_bias`/`mutate_af`/`mutate_aggrf`/`mutate_pf` config/profile mapping, reference-closer `mutate_weights` semantics via proportional multi-weight perturbation (`1/sqrt(total_weights)` + at-least-one fallback), dedicated directional add/remove link operators (`add_random_inlink`/`add_random_outlink`/`remove_random_inlink`/`remove_random_outlink`) wired through default mutation policy for `add_inlink`/`add_outlink`/`remove_inlink`/`remove_outlink` parity, explicit splice operators (`add_random_outsplice`/`add_random_insplice`) for `outsplice`/`insplice` parity, explicit sensor/actuator mutation operators (`add_random_sensor`/`add_random_sensorlink`/`add_random_actuator`/`add_random_actuatorlink`) filtered by scape-compatible IO registry entries, explicit substrate-structure operators (`add_random_cpp`/`add_random_cep`) for `add_cpp`/`add_cep` parity, and substrate-dimension circuit mutation operators (`add_circuit_node`/`add_circuit_layer`) for `add_CircuitNode`/`add_CircuitLayer` parity | `partial` | `internal/evo/mutations.go`, `internal/evo/mutations_test.go`, `cmd/protogonosctl/config.go`, `cmd/protogonosctl/profiles.go`, `pkg/protogonos/api.go`, `.ref/src/genome_mutator.erl` |
