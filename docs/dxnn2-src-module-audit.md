...
| `exoself.erl` | agent lifecycle + tuning + eval mode orchestration | `internal/tuning/exoself.go` + monitor integration with minimum-improvement acceptance threshold parity (`MIN_PIMPROVEMENT`-style), goal-aware early-stop wiring from monitor `fitness_goal`, perturbation controls (`perturbation_range`/`annealing_factor`), per-generation tuning telemetry in diagnostics/artifacts, and persisted derived efficiency ratios consumed by CLI diagnostics | `partial` | `internal/tuning/exoself.go`, `internal/tuning/exoself_test.go`, `internal/tuning/tuner.go`, `internal/evo/population_monitor.go`, `internal/evo/population_monitor_tuning_test.go`, `internal/platform/polis.go`, `internal/model/types.go`, `cmd/protogonosctl/main.go`, `cmd/protogonosctl/main_integration_test.go`, `cmd/protogonosctl/config.go`, `pkg/protogonos/api.go`, `.ref/src/exoself.erl` |
