...
| `exoself.erl` | agent lifecycle + tuning + eval mode orchestration | `internal/tuning/exoself.go` + monitor integration with minimum-improvement acceptance threshold parity (`MIN_PIMPROVEMENT`-style), goal-aware early-stop wiring from monitor `fitness_goal`, perturbation controls (`perturbation_range`/`annealing_factor`), per-generation tuning telemetry in diagnostics/artifacts, persisted derived efficiency ratios consumed by CLI diagnostics, tuning telemetry deltas in `species-diff`, optional from/to diagnostics snapshots via `species-diff --show-diagnostics`, and machine-readable emissions via `runs --json`, `lineage --json`, `top --json`, `species-diff --json`, and `diagnostics --json` | `partial` | `internal/tuning/exoself.go`, `internal/tuning/exoself_test.go`, `internal/tuning/tuner.go`, `internal/evo/population_monitor.go`, `internal/evo/population_monitor_tuning_test.go`, `internal/platform/polis.go`, `internal/model/types.go`, `cmd/protogonosctl/main.go`, `cmd/protogonosctl/main_integration_test.go`, `cmd/protogonosctl/config.go`, `pkg/protogonos/api.go`, `pkg/protogonos/api_test.go`, `.ref/src/exoself.erl` |
