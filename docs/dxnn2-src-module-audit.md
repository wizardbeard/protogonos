...
| `exoself.erl` | agent lifecycle + tuning + eval mode orchestration | `internal/tuning/exoself.go` + monitor integration with minimum-improvement acceptance threshold parity (`MIN_PIMPROVEMENT`-style), goal-aware early-stop wiring from monitor `fitness_goal`, perturbation controls (`perturbation_range`/`annealing_factor`), per-generation tuning telemetry in diagnostics/artifacts, persisted derived efficiency ratios consumed by CLI diagnostics, tuning telemetry deltas in `species-diff`, optional from/to diagnostics snapshots via `species-diff --show-diagnostics`, and machine-readable emissions via `runs --json`, `lineage --json`, `top --json`, `fitness --json`, `species --json`, `species-diff --json`, and `diagnostics --json` | `partial` | `internal/tuning/exoself.go`, `internal/tuning/exoself_test.go`, `internal/tuning/tuner.go`, `internal/evo/population_monitor.go`, `internal/evo/population_monitor_tuning_test.go`, `internal/platform/polis.go`, `internal/model/types.go`, `cmd/protogonosctl/main.go`, `cmd/protogonosctl/main_integration_test.go`, `cmd/protogonosctl/config.go`, `pkg/protogonos/api.go`, `pkg/protogonos/api_test.go`, `.ref/src/exoself.erl` |
| `neuron.erl` | neuron actor runtime, integrator/plasticity hooks | `internal/nn/network.go` with activation/aggregator forward path and reference-aligned output saturation (`[-1,1]`) | `partial` | `internal/nn/network.go`, `internal/nn/network_test.go`, `.ref/src/neuron.erl` |
...
| `genome_mutator.erl` | mutation operator suite | `internal/evo/mutations.go` with explicit reference-name mutators for `mutate_weights`, `add_bias`/`remove_bias`, `mutate_af`/`mutate_aggrf`, `mutate_pf`, `mutate_plasticity_parameters`, `add_neuron`, and `remove_neuron`; reference-closer `mutate_weights` semantics via proportional multi-weight perturbation (`1/sqrt(total_weights)` + at-least-one fallback); dedicated directional add/remove link operators for `add_inlink`/`add_outlink`/`remove_inlink`/`remove_outlink` without non-directional fallback when directional candidates are absent; explicit splice operators for `outsplice`/`insplice` with directional candidate gating; explicit sensor/actuator add/remove mutation operators for `add_sensor`/`add_sensorlink`/`add_actuator`/`add_actuatorlink`/`remove_sensor`/`remove_actuator` filtered by scape-compatible IO registry entries with split component-vs-link semantics (`add_*link` increments link counters, `remove_*` removes selected endpoint and all its link occurrences); explicit cutlink alias operators for `cutlink_FromElementToElement`/`cutlink_FromNeuronToNeuron`/`cutlink_FromSensorToNeuron`/`cutlink_FromNeuronToActuator`; explicit substrate-structure operators for `add_cpp`/`remove_cpp`/`add_cep`/`remove_cep`; substrate-dimension circuit mutation operators for `add_circuit_node`/`delete_circuit_node`/`add_circuit_layer` gated to existing substrate-configured genomes; legacy circuit mutation names are canonicalized during config/profile ingestion (`add_CircuitNode` -> `add_circuit_node`, etc.); explicit search-parameter mutator operators for `mutate_tuning_selection`/`mutate_tuning_annealing`/`mutate_tot_topological_mutations`/`mutate_heredity_type`; and operator-name parity aligned to reference tags for lineage/reporting output | `partial` | `internal/evo/mutations.go`, `internal/evo/mutations_test.go`, `cmd/protogonosctl/config.go`, `cmd/protogonosctl/profiles.go`, `pkg/protogonos/api.go`, `.ref/src/genome_mutator.erl` |
...
| `plasticity.erl` | Hebbian/Oja/etc plasticity rules | `internal/nn/plasticity.go` + cortex integration with runtime alias normalization for reference PF names (`hebbian_w`, `ojas`, `ojas_w`) | `partial` | `internal/nn/plasticity.go`, `internal/nn/plasticity_test.go`, `internal/agent/cortex.go`, `.ref/src/plasticity.erl` |
