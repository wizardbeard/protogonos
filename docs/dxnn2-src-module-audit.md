...
| `genome_mutator.erl` | mutation operator suite | `internal/evo/mutations.go` with dedicated bias/activation/aggregator perturbation operators and reference `add_bias`/`mutate_af`/`mutate_aggrf` config/profile mapping | `partial` | `internal/evo/mutations.go`, `internal/evo/mutations_test.go`, `cmd/protogonosctl/config.go`, `cmd/protogonosctl/profiles.go`, `pkg/protogonos/api.go`, `.ref/src/genome_mutator.erl` |
