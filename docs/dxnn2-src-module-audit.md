...
| `genome_mutator.erl` | mutation operator suite | `internal/evo/mutations.go` with dedicated bias add/remove, activation/aggregator, and plasticity-rule mutation operators, reference `add_bias`/`remove_bias`/`mutate_af`/`mutate_aggrf`/`mutate_pf` config/profile mapping, and reference-closer `mutate_weights` semantics via proportional multi-weight perturbation (`1/sqrt(total_weights)` + at-least-one fallback) wired through default mutation policy | `partial` | `internal/evo/mutations.go`, `internal/evo/mutations_test.go`, `cmd/protogonosctl/config.go`, `cmd/protogonosctl/profiles.go`, `pkg/protogonos/api.go`, `.ref/src/genome_mutator.erl` |
