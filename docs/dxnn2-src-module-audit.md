...
| `map2rec.erl` | map->record conversion helpers | `internal/map2rec` (broad baseline parity across core records including `constraint`/`pmp`/`sensor`/`actuator`/`neuron`/`agent`/`cortex`/`specie`/`population`/`trace`/`stat`/`topology_summary`/`signature`/`champion`) + runtime use in parity profile ingestion and `benchmark --config` ingestion | `partial` | `internal/map2rec/convert.go`, `internal/map2rec/convert_test.go`, `cmd/protogonosctl/profiles.go`, `cmd/protogonosctl/config.go`, `.ref/src/map2rec.erl` |
