...
| `map2rec.erl` | map->record conversion helpers | `internal/map2rec` (`constraint`/`pmp`/`sensor`/`actuator` parity + `neuron`/`agent`/`cortex`/`specie`/`population` materializers) | `partial` | `internal/map2rec/convert.go`, `internal/map2rec/convert_test.go`, `.ref/src/map2rec.erl` |
