...
func run(ctx context.Context, args []string) error {
	...
	case "comm-grid-mentor":
		return runCommGridMentor(ctx, args[1:])
...
func runRun(ctx context.Context, args []string) error {
	...
	commGridMentorProvider := fs.String("comm-grid-mentor-provider", "", "optional comm-grid-mentor provider: fixture|openai-compatible")
	commGridMentorBaseURL := fs.String("comm-grid-mentor-base-url", "", "OpenAI-compatible comm-grid-mentor base URL ending in /v1")
	commGridMentorAPIKeyEnv := fs.String("comm-grid-mentor-api-key-env", "PROTOGONOS_LLM_API_KEY", "environment variable containing comm-grid-mentor provider API key")
	commGridMentorModel := fs.String("comm-grid-mentor-model", "", "comm-grid-mentor provider model id")
	commGridMentorTimeoutMS := fs.Int("comm-grid-mentor-timeout-ms", 30000, "comm-grid-mentor provider request timeout in milliseconds")
	commGridMentorMaxTokens := fs.Int("comm-grid-mentor-max-tokens", 8, "comm-grid-mentor provider max tokens")
	commGridMentorTemperature := fs.Float64("comm-grid-mentor-temperature", 0, "comm-grid-mentor provider temperature")
	commGridMentorSeed := fs.Int64("comm-grid-mentor-seed", 0, "comm-grid-mentor provider seed when supported")
	...
	epitopesValidationEnd := fs.Int("epitopes-validation-end", 0, "optional epitopes validation end index for loaded CSV")
	epitopesTestStart := fs.Int("epitopes-test-start", 0, "optional epitopes test start index for loaded CSV")
	epitopesTestEnd := fs.Int("epitopes-test-end", 0, "optional epitopes test end index for loaded CSV")
	epitopesBenchmarkStart := fs.Int("epitopes-benchmark-start", 0, "optional epitopes benchmark start index for loaded CSV")
	epitopesBenchmarkEnd := fs.Int("epitopes-benchmark-end", 0, "optional epitopes benchmark end index for loaded CSV")
	...
	if *configPath == "" {
		req = protoapi.RunRequest{
			...
			CommGridMentorProvider:  *commGridMentorProvider,
			CommGridMentorBaseURL:   *commGridMentorBaseURL,
			CommGridMentorAPIKeyEnv: *commGridMentorAPIKeyEnv,
			CommGridMentorModel:     *commGridMentorModel,
			CommGridMentorTimeoutMS: *commGridMentorTimeoutMS,
			CommGridMentorMaxTokens: *commGridMentorMaxTokens,
			CommGridMentorTemp:      *commGridMentorTemperature,
			CommGridMentorSeed:      *commGridMentorSeed,
		...
		err := overrideFromFlags(&req, setFlags, map[string]any{
			...
			"comm-grid-mentor-provider":    *commGridMentorProvider,
			"comm-grid-mentor-base-url":    *commGridMentorBaseURL,
			"comm-grid-mentor-api-key-env": *commGridMentorAPIKeyEnv,
			"comm-grid-mentor-model":       *commGridMentorModel,
			"comm-grid-mentor-timeout-ms":  *commGridMentorTimeoutMS,
			"comm-grid-mentor-max-tokens":  *commGridMentorMaxTokens,
			"comm-grid-mentor-temperature": *commGridMentorTemperature,
			"comm-grid-mentor-seed":        *commGridMentorSeed,
...
func runCommGridMentorNormalCompare(ctx context.Context, client *protoapi.Client, req protoapi.RunRequest) error {
	...
	if strings.TrimSpace(req.CommGridMentorProvider) != "" {
		return errors.New("comm-grid mentor baseline comparison uses fixture plans only")
	}
...
func usageError(msg string) error {
	return fmt.Errorf("%s\nusage: protogonosctl <init|reset|start|run|benchmark|benchmark-experiment|profile|runs|lineage|fitness|diagnostics|species|species-diff|monitor|population|top|scape-summary|epitopes-test|comm-grid-llm|comm-grid-mentor|comm-grid-llm-suite|comm-grid-llm-runs|export> [flags]", msg)
}
